  ...
  return (
    <div className={styles.pizzaBlock}>
      ...
      <div className={styles.pizzaBlock__selector}>
        <ul>
          {types.map((type) => (
            <li key={type} onClick={() => setActiveType(type)} className={activeType === type ? styles.active : ''}>
              {typeNames[type]}
            </li>
          ))}
        </ul>
        <ul>
          {sizes.map((size, i) => (
            <li key={size} onClick={() => setActiveSize(i)} className={activeSize === i ? styles.active : ''}>
              {size} см.
            </li>
          ))}
        </ul>
